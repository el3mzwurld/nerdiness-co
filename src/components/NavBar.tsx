...
const NavBar = ({ links, windowWidth, routes }: NavProps) => {
  const navigate = useNavigate();

  const handleNavtoHome = () => {
    navigate({ to: "/" });
  };
  const navToRoute = (route: string): void => {
    navigate({ to: route });
  };
  ...
  return (
    <div className="w-full px-2.5 py-2.5 flex items-center justify-between lg:bg-transparent bg-[#50052D] absolute top-0 left-0 z-1">
      ...
      {windowWidth < 1024 && isNavOpen && (
        <motion.nav
          className={`${isNavOpen ? "nav-open" : ""} absolute -translate-x-1/2 bg-white left-[50%] flex flex-col w-[50%] h-auto z-1 items-center justify-center gap-2 list-none py-[10px] text-[12px] font-medium text-gray-500 rounded-md`}
